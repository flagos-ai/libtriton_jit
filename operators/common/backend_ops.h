...
inline RawStream get_device_stream([[maybe_unused]] const at::Tensor& t) {
    ...
    return static_cast<CUstream>(
        c10::cuda::getCurrentCUDAStream(t.device().index()).stream());
#endif
}

// ---- Tensor allocation (wraps MUSA musaMalloc difference) ----
inline at::Tensor backend_empty(
    at::IntArrayRef sizes, at::ScalarType dtype, at::Device device) {
#if defined(BACKEND_MUSA)
    void* p = nullptr;
    size_t bytes = 1;
    for (auto s : sizes) bytes *= static_cast<size_t>(s);
    bytes *= at::elementSize(dtype);
    if (musaMalloc(&p, bytes) != musaSuccess)
        throw std::runtime_error("musaMalloc failed");
    return at::from_blob(p, sizes, [](void* ptr) { musaFree(ptr); },

...
struct CorrectnessResult {
    ...
    CorrectnessResult() : passed(false), max_abs_diff(0), max_rel_diff(0),
                          num_mismatches(0), total_elements(0), message("") {}
};
...
struct BenchmarkResult {
    ...
    BenchmarkResult() : mean_latency_us(0), std_latency_us(0), min_latency_us(0),
                        max_latency_us(0), throughput_gbps(0), tflops(0),
                        warmup_iters(0), bench_iters(0), operator_name("") {}
};
...
struct TestConfig {
    ...
    TestConfig() : dtype(at::kFloat), atol(1e-5), rtol(1e-5),
                   warmup_iters(10), bench_iters(100), test_name("") {}
};
...
public:
     ...
     */
    at::Device get_device() const { return device_; }
...
public:
     ...
     */
    at::Tensor rand(const std::vector<int64_t>& shape, double low, double high,
                    at::ScalarType dtype = at::kFloat);

    /**
     * @brief Create a random integer tensor
     */
    at::Tensor randint(const std::vector<int64_t>& shape, int64_t low, int64_t high,
     ...
     */
    at::Tensor full(const std::vector<int64_t>& shape, double value,
                    at::ScalarType dtype = at::kFloat);
...
public:
    ...
    CorrectnessResult run_correctness_test(const TestConfig& config,
                                           OpFunc&& op_func,
                                           RefFunc&& ref_func);
...
CorrectnessResult TestRunner::run_correctness_test(const TestConfig& config,
    ...
    try {
        ...
        // Compare results
        result = CorrectnessChecker::detailed_compare(
            actual, expected, config.atol, config.rtol);
...
BenchmarkResult TestRunner::run_benchmark(const TestConfig& config,
    ...
    try {
        ...
        result.std_latency_us = std::sqrt(sum_sq / latencies.size() -
...
#define TEST_ASSERT_NEAR(a, b, tol, msg) \
    if (std::abs((a) - (b)) > (tol)) { \
        std::cerr << "ASSERTION FAILED: " << msg << " (" << a << " vs " << b << ") at " << __FILE__ << ":" << __LINE__ << std::endl; \
        return 1; \
    }

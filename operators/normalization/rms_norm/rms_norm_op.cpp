...
at::Tensor rms_norm(const at::Tensor& input, const at::Tensor& weight, double eps) {
    ...
    size_t out_bytes = n_rows * hidden_size * at::elementSize(input.scalar_type());
...
TORCH_LIBRARY(rms_norm_ops, m) {
    ...
    TORCH_LIBRARY_IMPL(rms_norm_ops, PrivateUse1, m) {
        m.impl("rms_norm", TORCH_FN(rms_norm));
    }
#else
    TORCH_LIBRARY_IMPL(rms_norm_ops, CUDA, m) {
        m.impl("rms_norm", TORCH_FN(rms_norm));
    }
#endif

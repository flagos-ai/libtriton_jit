...
struct IxBackend {
  ...
  static void launch_kernel(CUstream stream,
    ...
    if (result != CUDA_SUCCESS) {
      ...
      throw std::runtime_error(fmt::format("IX kernel launch failed: {}", error_string));
    }
  }
  ...
  static int get_device_index() {
    ...
    if (result != CUDA_SUCCESS) {
      const char* error_string;
      cuGetErrorString(result, &error_string);
      throw std::runtime_error(fmt::format("Failed to get IX device: {}", error_string));
    }
  ...
  static CUfunction load_kernel(const std::string& dir, const std::string& kernel_name) {
    ...
    LOG(INFO) << fmt::format("Loading IX kernel {} with arch={}, shared={}",
                             kernel_name,
                             metadata.arch,
                             metadata.shared);
 ...
 private:
  static void configure_shared_memory(CUfunction kernel, unsigned int required_shared) {
    CUdevice device;
